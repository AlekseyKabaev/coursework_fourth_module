...
def filtered_pay_range(pay_range):
    ...
    for j in sorted_list:
        if j['salary_from'] == 0 and pay_from <= j['salary_to'] <= pay_to and j not in res_list:
            res_list.append(j)
        elif pay_from <= j['salary_from'] <= pay_to and j['salary_to'] == 0 and j not in res_list:
            res_list.append(j)
        elif pay_from <= j['salary_from'] and j['salary_to'] <= pay_to and j not in res_list:
            res_list.append(j)
    return res_list
